...
contract TronUs {
	...
	uint256 constant public INVEST_MAX_AMOUNT = 50000e6;
	uint256 constant public WITHDRAW_MIN_AMOUNT = 200e6;
	uint256 constant public WITHDRAW_MAX_AMOUNT = 10000e6;
	uint256 constant public BASE_MULTIPLE = 3;
	uint256 constant public BASE_PERCENT = 100;
	uint256 constant public DIRECT_BONUS_PERCENT = 1000;
	uint256 constant public WITHDRAW_FEE_PERCENT = 1000;
	uint256 constant public FEE_INVEST_PERCENT = 500;
	uint256 constant public PERCENTS_DIVIDER = 10000;
	uint256 constant public TIME_STEP = 1 days;

	uint256 constant public INVEST_FEE_PERCENT = 1000;
	uint256 constant public WITHDRAW_SERVICE_FEE = 500;
	uint256[10] public REFERRAL_PERCENT = [500, 500, 300, 300, 300, 300, 100, 100, 100, 100];
	...
	uint256[2] public directProfit = [400e6, 1600e6];
	uint256 public initBonusAmount = 100e6;
	uint256 public initBonusLeft = 1000;

	address payable public feeInvestRec0;
	address payable public feeInvestRec1;
	address payable public feeWithdrawRec;
	...
	constructor(address payable _feeInvestRec0, address payable _feeInvestRec1, address payable _feeWithdrawRec) public {
		feeInvestRec0 = _feeInvestRec0;
		feeInvestRec1 = _feeInvestRec1;
	...
	function invest(address _referrer) public payable {
		...
		feeInvestRec0.transfer(investFee.mul(15).div(100));
		feeInvestRec1.transfer(investFee.mul(85).div(100));
		...
		uint256 reinvestAmount = dividends.mul(BASE_MULTIPLE).sub(dividends);
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0)) {
					...
					uint256 bonus = _investAmount.mul(REFERRAL_PERCENT[i]).div(PERCENTS_DIVIDER);
